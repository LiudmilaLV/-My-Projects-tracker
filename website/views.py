...
def home():
    ...
    if project_form.validate_on_submit():
        user = Project.query.filter(
            Project.project_name == project_form.project_name.data, Project.user_id == current_user.id).first()
        if user:
            flash('Project already exists! Use another name', category='warning')
        else:
            new_project = Project(project_name=project_form.project_name.data,
                                  goal=project_form.goal.data, notes=project_form.notes.data, user_id=current_user.id)
...
def project(project_id):
    ...
    current_project = Project.query.filter(
        Project.id == project_id, Project.owner == current_user).first_or_404()
    if current_project.owner != current_user:
        abort(403)

    # Get last 5 queries made by the user to show in a table on the page
    last5 = Entry.query.join(Project).filter(and_(Entry.project_id == current_project.id,
                                                  Project.owner == current_user)
                                             ).order_by(Entry.timestamp.desc()).limit(5).all()
    current_day = datetime.now().date()

    # Get data for a "this week" chart
    last_monday = current_day - timedelta(days=current_day.weekday())
    current_week = last_monday.strftime(
        "%b %e") + ' - ' + (last_monday + timedelta(days=6)).strftime("%b %e")
    ...
    next_monday = current_day - \
        timedelta(days=current_day.weekday()) + timedelta(days=7)
    ...
    last_monday = current_day - timedelta(days=current_day.weekday())
    reversed_12weeks_l = [last_monday.strftime(
        "%b %e") + ' - ' + (last_monday + timedelta(days=6)).strftime("%b %e")]
    for week in range(1, 12):
        reversed_12weeks_l.append((last_monday - timedelta(days=7 * week)).strftime("%b %e") + ' - ' + (
            last_monday - timedelta(days=7 * week) + timedelta(days=6)).strftime("%b %e"))
    ...
    week_of_year_for_labels = [
        [last_monday.isocalendar()[1], last_monday.isocalendar()[0]]]
    for week in range(1, len(these_12weeks_l)):
        week_of_year_for_labels.append([(last_monday - timedelta(days=7 * week)).isocalendar()[
                                       1], (last_monday - timedelta(days=7 * week)).isocalendar()[0]])
    ...
    these_12weeks_d = adjust_data(
        week_of_year_for_data, raw_these_12weeks_d, week_of_year_for_labels)
    ...
    if current_project.goal:
        ...
        for week, _ in enumerate(these_12weeks_d):
            if week == 0:
                continue
            these_12weeks_p.append(
                these_12weeks_d[week] + these_12weeks_p[week - 1])
...
def add_time(project_id):
    ...
    if entry_form.validate_on_submit():
        new_entry = Entry(date=entry_form.date.data,
                          duration=entry_form.duration.data, project_id=current_project.id)
        db.session.add(new_entry)
        db.session.commit()
        plural = ''
        if entry_form.duration.data != 1:
            plural = 's'
        flash(f'{entry_form.duration.data} minute{plural} added to the project!',
              category='success')
...
def delete_project(project_id):
    ...
    if current_project.owner != current_user:
        abort(403)
    current_project_entries = Entry.query.filter(
        Entry.project_id == project_id).all()
    for entry in current_project_entries:
        db.session.delete(entry)
    db.session.delete(current_project)
    db.session.commit()
    flash(
        f'Project "{current_project.project_name} has been deleted!', category='success')
...
def delete_entry(entry_id):
    current_entry = Entry.query.join(Project).filter(
        and_(Entry.id == entry_id, Project.owner == current_user)).first_or_404(entry_id)
    db.session.delete(current_entry)
    db.session.commit()
    plural = ''
    if current_entry.duration != 1:
        plural = 's'
    flash(f'{current_entry.duration} minute{plural} has been deleted!',
          category='success')
